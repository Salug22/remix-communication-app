...
self.addEventListener("push", (event) => {
  ...
  const title = data2.title || "Default Title";
  const options = {
    body: data2.body || "Default body text",
    icon: data2.icon || "/192x192.png"
  };
  event.waitUntil(self.registration.showNotification(title, options));
});
self.addEventListener("message", (event) => {
  var _a;
  if (((_a = event.data) == null ? void 0 : _a.type) === "PUSH_TEST") {
    const data2 = JSON.parse(event.data.payload);
    const title = data2.title || "Default Title";
    const options = {
      body: data2.body || "Default Body",
      icon: data2.icon || "/192x192.png"
    };
...
const routes = {
  ...
  "routes/synonyme-list": {
    id: "routes/synonyme-list",
    parentId: "root",
    path: "synonyme-list",
    index: void 0,
    caseSensitive: void 0,
